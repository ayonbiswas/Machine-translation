...
for j in range(source_sequence_length):
    enc_train_inputs.append(tf.placeholder(tf.int32, shape=[batch_size],name='enc_train_inputs_%d'%j))

dec_train_labels=[]
dec_label_masks = []
for j in range(target_sequence_length):
    dec_train_inputs.append(tf.placeholder(tf.int32, shape=[batch_size],name='dec_train_inputs_%d'%j))
    dec_train_labels.append(tf.placeholder(tf.int32, shape=[batch_size],name='dec-train_outputs_%d'%j))
    dec_label_masks.append(tf.placeholder(tf.float32, shape=[batch_size],name='dec-label_masks_%d'%j))
...
#defining model saver
    ...
    if (step+1)%500==0:
        ...
        save_path = saver.save(sess, "/home/nier/Documents/ml project/logs/test2model")
